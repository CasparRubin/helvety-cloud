...
Implement P6b only. Projects + issues under a workspace: list/create/reorder projects; issue list + detail with encrypted title/body (AES-GCM + AAD); list/paginate /api/v1 endpoints; decrypt with workspace_key from wrapped_keys. Soft-delete if schema supports. Free-tier/greenfield.

No TipTap, notes/contacts, sharing, Stripe.
```

**Done when:** Create/edit/list/reload issues across sessions; API/DB stay ciphertext-opaque.
