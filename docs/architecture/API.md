...
| PUT            | `/api/v1/me/policy-acceptances`                                               | Record ToS/Privacy/AUP/E2EE/eligibility version acceptances                                                                                         |
...
| PUT/GET/DELETE | `/api/v1/workspaces/:workspaceId/notes/:noteId`                               | Note upsert / fetch / delete (`links` replace non-project outgoing edges; optional `projectIds` replace project affiliations)                       |
| GET            | `/api/v1/workspaces/:workspaceId/boards`                                      | List boards (paginated; ciphertext-opaque)                                                                                                          |
| PUT/GET/DELETE | `/api/v1/workspaces/:workspaceId/boards/:boardId`                             | Board upsert / fetch / delete (`links` replace outgoing note/task/contact/project/database/table placements; create gated)                          |
| GET            | `/api/v1/workspaces/:workspaceId/databases`                                   | List databases (paginated; ciphertext-opaque; P18)                                                                                                  |
| PUT/GET/DELETE | `/api/v1/workspaces/:workspaceId/databases/:databaseId`                       | Database upsert / fetch / delete (create gated; DELETE clears table + database link edges)                                                          |
| GET            | `/api/v1/workspaces/:workspaceId/databases/:databaseId/tables`                | List tables under a database (paginated; ciphertext-opaque)                                                                                         |
| PUT/GET/DELETE | `/api/v1/workspaces/:workspaceId/databases/:databaseId/tables/:tableId`       | Table upsert / fetch / delete (create gated per database; DELETE clears table link edges)                                                           |
...
| GET            | `/api/v1/me/invitations`                                                      | Pending invitations for the caller’s verified email (`accepted_at` / `cancelled_at` null)                                                           |
| POST           | `/api/v1/me/invitations/:invitationId/claim`                                  | Invitee attaches their `public_key` (must match their `user_crypto` row)                                                                            |
| POST           | `/api/v1/me/invitations/:invitationId/accept`                                 | Atomic membership + `wrapped_keys` insert; clears invitation seal after copy                                                                        |
...
| PUT            | `/api/v1/workspaces/:workspaceId/billing/addons`                              | Any member: set Capacity Increase quantity (0–20) on a Pro Workspace Stripe subscription                                                            |
...
| POST           | `/api/v1/workspaces/:workspaceId/attachments/:attachmentId/complete`          | Verify Storage size vs declared `byteSize`; ready on match; on failure delete object (if present) and mark failed                                   |
...
**Attachments (P11):** Client encrypts file bytes with a per-file DEK, wraps the DEK under `workspace_key`, and stores filename/mime in `encryptedMeta`. Server stores opaque envelopes + Storage ciphertext only. TipTap `fileAttachment` atoms and `attachment_links` join notes/tasks/contacts to attachment ids without decrypting bodies. Upload/create is entitlement-gated (storage + attachment counts); see [`BILLING.md`](./BILLING.md). Complete rejects uploads whose Storage size is missing or does not match `byteSize`, and removes the Storage object so failed rows do not leave quota-uncounted blobs.

**Request body caps (abuse brake):** Ciphertext envelope payloads are limited to ~1.5 MiB decoded (`packages/api-contract`). PUT `links` arrays are capped at 50; `attachmentIds` at 20. Oversize bodies return `invalid_body`.

**Invitation lifecycle (P6e):** `waiting_for_recipient` → `waiting_for_seal` → `ready_to_accept` → `accepted` (or `cancelled`). Any email is invitable; invitee signs in with OTP, sets up encryption, claims, then any member seals `workspace_key` to the claimed public key with AAD `wrapped_keys:{workspaceId}:wrapped_key`. Claim stores the caller’s registered `user_crypto.public_key`, so seals can only target the invitee’s own encryption key. Invitation payloads expose `workspaceEncryptedBlob` (not a plaintext workspace name); the invitee decrypts the name after seal when they hold the workspace key. Accept copies the seal into `wrapped_keys` then **clears** `sealed_workspace_key` on the invitation (same as cancel). Leave/remove also null any leftover seals for that member. `GET /me/invitations` returns pending invitations only (`accepted_at` is null). A sealed key already opened by the invitee is not recoverable, so rotation stays a later concern. Server never sees plaintext keys. Inviting someone grants full peer rights (invite, remove, billing, delete).
...
**Task categorizations (P7/P8d/P8e/P14):** Workspace `encrypted_blob` includes `categorizations` (encrypted names, optional option `color` / `icon`, optional stage `completionPercent` 0–100). Tasks store `labelId` (nullable), `stageId`, `priorityId` as plaintext soft refs. No separate categorization API; defs ride on workspace PATCH/GET. Client-side weighted progress averages stage weights over non-Cancelled tasks.

**Entity links:** Note / task / contact PUT may include `links: [{ kind, id }]`. Allowed body pairs are note–task/contact/database/table and contact–note/task/database/table (plus task–note/contact/database/table); project affiliations for notes and contacts use optional `projectIds: uuid[]` (0..n) and are stored as `entity_links` with `target_kind = project`. Board PUT may include `links` for placed note/task/contact/project/database/table nodes (board–entity pairs). Databases and tables do not emit outgoing links. When `links` is provided, the server replaces **non-project** outgoing edges only (boards replace all outgoing edges for that board). When `projectIds` is provided on notes/contacts, it replaces **project** outgoing edges only. The server validates pair rules and workspace ownership. Responses include all outgoing links, and `GET …/links` supports backlinks. UUID edges are intentional metadata; titles and colors stay in ciphertext. Inline TipTap `entityRef` nodes remain inside note/task/contact body ciphertext and are extracted on save (project refs are not written from the body). Board canvas edges stay in board ciphertext only.

**Entitlement gates (P6f / P12 / P16 / P17 / P18):** create mutations (new workspace/project/task/note/contact/comment/board/database/table, invite create, invite accept, attachment upload / new attachment links) are gated by **effective** workspace limits (`BILLING.md`) and return `limit_exceeded` (403) at the cap. Tasks are per-project. Tables are per-database. Comments + replies share one workspace meter. Board shapes per board are catalogued and client-enforced only (ciphertext). Soft-locked free-overflow workspaces (`freeOverflowLocked` on GET billing) also block those creates while leaving reads/updates/deletes/export available. Updates, soft-deletes, reads, seal/cancel, and billing actions are never gated by overflow. Meters are plaintext row counts only.
...
Route handlers use Supabase client with the **user JWT**. Service role is used by `/api/webhooks/stripe` (`apps/web/lib/supabase/service-role.ts`) to upsert `subscriptions` / `billing_events`, by attachment Storage admin helpers, by free-overflow soft-lock creator-wide reads after membership proof, by project wrap cleanup after workspace verification, and by `DELETE /api/v1/me` for `auth.admin.deleteUser`, never to “helpfully” decrypt or touch encrypted entity tables.
