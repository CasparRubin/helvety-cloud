...
| `workspaces` | Workspace ids, `kind` (`personal` \| `standard`), `created_by` (free-slot attribution + timestamps); at most one Personal per `created_by`. `encrypted_blob` holds the workspace name plus workspace-scoped task categorizations. |
...
| `workspace_invitations` | Email-targeted invites: normalized `email`, role always `member`, claim (`claimed_by`, `claimed_public_key`; always the claimer’s `user_crypto.public_key`), member-produced `sealed_workspace_key` (cleared on cancel), accept/cancel timestamps |
