...
| Table | Content |
|-------|---------|
| `projects` | `encrypted_blob` holds `{ name, categorizations, color? }` where `categorizations` has `labels` / `stages` / `priorities` arrays of `{ id, name, sortOrder, color?, isDefault? }` (names + colors encrypted); stage `color` is an `EntityColor` palette token (P8d — seeds defaults for default stage names); optional top-level `color` is a palette token (P8c); plaintext FKs: `id`, `workspace_id`, sort, timestamps, tombstone |
