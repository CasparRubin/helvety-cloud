/**
 * WebAuthn PRF for encryption unlock (session ≠ decrypt).
...
/**
 * First-time encryption setup: create a PRF-capable discoverable credential and derive unlock_key.
 */
export async function createPrfUnlock(
  ...
  try {
    ...
    if (!asserted) {
      throw new Error("PRF assertion was cancelled");
    }
