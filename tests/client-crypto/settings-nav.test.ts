...
import {
  ...
  parseAppNavPath,
  resolveNavBackMode,
} from "../../apps/web/lib/app-nav-path";
import { workspaceSettingsNavItems } from "../../apps/web/lib/settings-nav";

const t = (key: string) => key;

describe("settings nav items", () => {
  it("hides workspace danger for non-owners", () => {
    const withDanger = workspaceSettingsNavItems("ws1", t);
    const withoutDanger = workspaceSettingsNavItems("ws1", t, {
      showDanger: false,
    });
